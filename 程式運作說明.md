...
3. **主迴圈**（`while self.reading`）：
   ...
   b. **資料讀取策略**（根據資料長度決定讀取方式）：
      
      - **如果 `vib_data[0] >= max_size`（123）**：
        - 讀取位址 `0x02` 的 `max_size + 1` 個暫存器（一次性讀取大量資料）
        - 設定 `this_len = max_size`
      
      - **如果 `vib_data[0] <= 6`**：
        - 等待 1ms（避免頻繁讀取）
        - 只讀取 1 個暫存器（資料長度）
        - 更新 `this_len`
        - `continue`（跳過本次處理，等待更多資料）
      
      - **其他情況**：
        - 讀取 `vib_data[0] + 1` 個暫存器（根據上次資料長度動態調整）
        - 更新 `this_len`
...
**運作流程**：
1. 取得資料鎖定（`data_lock`）
2. 將新資料附加到 `realtime_data`
3. 更新資料點計數器
4. **記憶體管理**：如果資料超過 10000 個點，只保留最近 10000 個點（避免記憶體過載）
...
| 檔案 | 說明 | 行數 | 主要功能 |
|------|------|------|----------|
| `main.py` | 主控制程式 | 300 | Flask Web 伺服器、執行緒管理、資料收集迴圈 |
| `prowavedaq.py` | 硬體通訊模組 | 408 | Modbus RTU 通訊、資料讀取、資料轉換 |
| `csv_writer.py` | CSV 寫入器 | 110 | CSV 檔案建立、資料寫入、分檔邏輯 |
| `templates/index.html` | 主頁面 | 350 | 即時圖表、控制介面、JavaScript 邏輯 |
...
if vib_data[0] >= max_size:
    # 資料量大，一次性讀取
    result = self.client.read_input_registers(address=0x02, count=max_size + 1)
elif vib_data[0] <= 6:
    # 資料量小，等待更多資料
    time.sleep(0.001)
    ...
    # 根據上次資料長度動態讀取
    result = self.client.read_input_registers(address=0x02, count=vib_data[0] + 1)
```
